...
- `_worker.js` — Cloudflare Pages Function entry point.
  - `/api` takes `ipb_member_id` / `ipb_pass_hash` (and an optional `cf_connecting_ip`), uses `fetch()` on the forums to validate the credentials and check account suspension, then does the same to e-hentai.org as a fallback. For a valid account it uses `connect()` to talk directly to the ExHentai origin, implementing TLS 1.3 + HTTP by hand, with a `CF-Connecting-IP` header spoofing an egress IP from any country. It connects directly to the E-Hentai origin when ExHentai does not report the browsing country. Passing `?fetch=1` A/B-switches the origin lookups back to plain `fetch()`, which per Cloudflare's docs sets `CF-Connecting-IP` to a fixed IPv6 egress (`2a06:98c0:3600::103`, geolocated as the UK) — no country spoofing.
  - `/api/scan` powers the country scanner: it runs a plain-fetch pre-flight check against e-hentai.org, then probes each country in 27 random-IP countries, or all ~249 when the request sets `all_countries` over a **single** TLS/HTTP/2 connection to `s.exhentai.org` — each country is a separate multiplexed stream carrying its own spoofed `CF-Connecting-IP`, fired in waves — and streams each result back as NDJSON.
...
- `lib/directTls.js` — the `cloudflare:sockets`-based direct-connect implementation: opens a TCP socket, does the TLS 1.3 handshake via subtls, then speaks HTTP/2 by hand (ALPN-negotiated, HPACK-encoded frames), demultiplexing frames by stream ID so one connection can carry several concurrent requests.
...
- `country-ips.js` — an ES module (imported by both `_worker.js` and the two frontend pages) holding the country name -> probe network map used by the `CF-Connecting-IP` picker and the scan, plus `COUNTRY_ZH` (Chinese names) and `COUNTRY_CODE` (name -> ISO alpha-2, bridging country names to the map's CSS classes). 27 countries skip those narrow probe networks entirely and get a fresh random address per request from a real VPN egress range, via `spoofIpForCountry()`: the United States goes through `generateUsWarpIp()` (Cloudflare WARP), and 26 others through `generateFastlyVpnIp()`, which draws from that country's own Fastly MASQUE (Firefox VPN) `/44` blocks in `FASTLY_VPN_GROUPS`.
