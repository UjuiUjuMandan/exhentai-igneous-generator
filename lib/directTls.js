...
// SNI name and `hostHeader` as the HTTP virtual host - like curl's
...
// (`/certs.index.json` + `/certs.binary.txt`). Returns a `request()` you can
// call more than once to send several HTTP/1.1 requests over the one TLS
// connection, and a `close()` to call when done.
export async function openDirectHttpsSession({ origin, connectHost, hostHeader, sniHost = hostHeader, useSNI = true, port = 443 }) {
  ...
  const { read, write } = await startTls(sniHost, rootCerts, networkRead, networkWrite, {
    useSNI,
