...
} from '../vendor/subtls/src/util/readQueue.ts';
...
function getRootCerts(origin) {
  rootCertsPromise ??= (async () => {
    ...
    if (!indexRes.ok || !dataRes.ok)
      throw new Error('Failed to load root certificate database');
...
async function readHPACKHeaders(hpackBytes, length) {
  ...
  while (hpackBytes.offset < end) {
    ...
    if (byte & 0x80) {
      ...
      if (!entry)
        throw new Error(`Unsupported dynamic-table index ${tableIndex}`);
      [key, value] = entry;
    } else {
  ...
  async _pump() {
    try {
      for (;;) {
        ...
        const { payloadEnd, payloadRemaining, frameType, flags, streamId } =
        ...
        switch (frameType) {
          ...
          case HTTP2FrameType.RST_STREAM: {
            ...
            if (stream) {
              ...
              stream.reject(
                new Error(
                  `Stream ${streamId} reset by server (error ${errorCode})`,
                ),
              );
            }
            break;
          ...
          case HTTP2FrameType.DATA: {
            ...
            if (
              ...
              frameType === HTTP2FrameType.CONTINUATION
            ) {
  ...
  async request({ method = 'GET', path = '/', authority, headers = {} }) {
    ...
    return {
      ...
      body: textDecoder.decode(
        concatChunks(stream.bodyChunks, stream.bodyLength),
      ),
    };
  }
}
...
export async function openDirectHttpsSession({
  origin,
  connectHost,
  hostHeader,
  sniHost = hostHeader,
  ...
  const networkWrite = (data) => {
    writer.write(data);
  };

  const { read, write, protocolFromALPN } = await startTls(
    sniHost,
    rootCerts,
    networkRead,
    networkWrite,
  ...
  if (protocolFromALPN !== 'h2')
    throw new Error(
      `Server did not negotiate h2 (got ${protocolFromALPN ?? 'nothing'})`,
    );

  // HTTP/2 flow control: the default receive window is only 65535 bytes, per
  // stream AND for the connection as a whole. This client never sends
  // WINDOW_UPDATE frames as it consumes DATA, so once that window is spent the
  // server stops sending and response bodies are silently truncated. With a
  // single request that usually still covers a whole page, but when many
  // streams are multiplexed over one connection (the country scan) they share
  // the connection window and each body gets cut short - enough to carry the
  // headers (set-cookie/igneous) but not the deeper body text. So we open both
  // windows wide up front: advertise a large SETTINGS_INITIAL_WINDOW_SIZE for
  // every stream, and bump the connection-level window with one big
  // WINDOW_UPDATE. 2^30-1 is the maximum a window may hold (RFC 9113 6.9).
  const LARGE_WINDOW = 0x3fffffff; // 1 GiB - 1, the largest legal window
  const CONNECTION_WINDOW_INCREMENT = LARGE_WINDOW - 65535; // add on top of the default 65535
  ...
  preface.writeUint16(0x0004); // SETTINGS_INITIAL_WINDOW_SIZE
  preface.writeUint32(LARGE_WINDOW); // per-stream receive window
  endSettingsFrame();
  // Connection-level window only grows via WINDOW_UPDATE, not SETTINGS.
  const endWindowUpdate = writeFrame(
    preface,
    HTTP2FrameType.WINDOW_UPDATE,
    0x0,
  );
  preface.writeUint32(CONNECTION_WINDOW_INCREMENT);
  endWindowUpdate();
